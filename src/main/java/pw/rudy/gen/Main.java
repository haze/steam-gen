...
public class Main {
    ...
    public static void main(String... args) {
        ...
        System.out.println("Made by latematt, Rudy and Apteryx");
        System.out.println();
    ...
    private static void saveAccounts() {
        ...
        accountList.stream().forEach(acc -> accountsToWrite.append(String.format("http://steamcommunity.com/profiles/%s - %s - %s - %s", acc.getId(), acc.getProfileName(), acc.guessEmail(), (parser.isPresent("--setupcheck", Optional.<String>empty()) ? isSetup(String.format("http://steamcommunity.com/profiles/%s", acc.getId())) : ""))).append("\r\n"));
        try {
    ...
    private static void generateAccounts(StringBuilder builder) {
        ...
        for (JsonElement e : array) {
            ...
            if (accountList.contains(acc))
                continue;
            if (parser.isPresent("--verbose", Optional.<String>empty())) {
                System.out.println(String.format("#%s / #%s / %s - http://steamcommunity.com/profiles/%s - %s - %s - %s", index, accountList.size(), accountsNeeded, acc.getId(), acc.getProfileName(), acc.guessEmail(), (parser.isPresent("--setupcheck", Optional.<String>empty()) ? isSetup(String.format("http://steamcommunity.com/profiles/%s", acc.getId())) : "...")));
            }
            accountList.add(acc);
            index++;
        }
